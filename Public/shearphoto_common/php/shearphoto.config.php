...
//DIRECTORY_SEPARATOR 是斜杠符，为兼容WINDOW和LINUX

$ShearPhoto["config"] = array("proportional" => 1 / 1, //比例截图，如果你是动态比例，请设为false，false时就不会验证JS端的比例了。如果为数字时，JS端也要相应设置哦，不然系统会给你抱出错误,不设比例填0，如填比例 ：3/4  代表宽和高的比例是3/4(3除以4的意思懂吗，菜菜，你可直接填0.75，没错)

"quality" => 100, // 截图质量，0为一般质量（质量大概75左右），  0-100可选 ！ 整数型，质量越高，越清淅，缺点是文件体积越大，不是太严格追求图片高清，设0就可以,提示：PNG图片不带此效果
...
"saveURL" => $_SERVER['DOCUMENT_ROOT']."/public/uploads/users/", //截好后的图片。储存的目录位置，后面不要加斜杠，系统会自动给补上！不要使用中文

"filename" => uniqid("avatar") . "_" . mt_rand(100, 999) . "_"//截好后的图片的文件名字定义！要生成多个文件时 系统会自动在后面补上  "width"=>array()参数定义的名称,请查看上面的"width"=>array()
);
