...
class Auth{
    ...
    protected function getAuthList($uid,$type) {
        ...
        //读取用户组所有权限规则
        $rules = $this->CI->db->from($this->_config['AUTH_RULE'])->where_in('id',$ids)->get()->result_array();
        //循环规则，判断结果。
        $authList = array();   //
    ...
    public function getUserInfo($uid) {
        ...
        if(!isset($userinfo[$uid])){
            $userinfo[$uid] = $this->CI->db->from($this->_config['AUTH_USER'])->where(array('id'=>$uid))->get()->row_array();
        }
        return $userinfo[$uid];
    }
}
