...
class Form {
    ...
    */
    public static function checkbox($array = array(), $id = '', $str = '', $defaultvalue = '', $field = '') {
        ...
        foreach($array as $key=>$value) {
            ...
            $string .= '<label><input type="checkbox" '.$str.' id="'.$field.'_'.$i.'" '.$checked.' value="'.htmlspecialchars($key).'"> '.htmlspecialchars($value).'</label>';
            $i++;
        }
        return $string;
    }
    ...
    * @param $str 属性
    */
    public static function radio($array = array(), $id = 0, $str = '', $field = '') {
        $string = '';
        foreach($array as $key=>$value) {
            $checked = trim($id)==trim($key) ? 'checked' : '';
            $string .= '<label><input type="radio" '.$str.' id="'.$field.'_'.htmlspecialchars($key).'" '.$checked.' value="'.$key.'"><i></i> '.$value.'</label>';
        }
        return $string;
    }
    /**
